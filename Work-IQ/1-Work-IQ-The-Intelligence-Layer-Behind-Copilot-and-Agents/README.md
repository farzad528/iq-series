# Episode 1: Work IQ: The Intelligence Layer Behind Copilot & Agents

[![Episode 1 video](../../images/work-iq/ep1.png)](https://aka.ms/iq-series/work-iq/episode1)

📅 **June 2, 2026 at 9 AM PT** | 📺 [Watch the session](https://aka.ms/iq-series/work-iq/episode1) | 📂 [Try the lab](./lab/)


## 🎥 Session Summary

### 🎬 Executive Introduction

Rob Howard introduces Work IQ and sets the stage for the series.

### 💬 Tech Talk

Paolo Pialorsi and Tolga Kilicli walk through the idea of Work IQ, explaining:

- Overview of Work IQ
- Architecture of Work IQ
- Protocol strategy of Work IQ (REST, A2A, MCP)

### 🖍 Doodle Summary

A visual summary of key takeaways by Tomomi Imura, showing what Work IQ is and how it fits into the agents landscape.

![Doodle summary Episode 1](../../images/work-iq/E1-recap.png)

## � Try the Lab

Ready to get hands-on? Head to the [Episode 1 lab](./lab/) for prerequisites, deployment instructions, and a step-by-step guidance.

## 🔗 Learn More

- 📖 [Work IQ overview](https://learn.microsoft.com/en-us/microsoft-365/copilot/extensibility/work-iq)
- 📚 [Copilot Dev Camp - Work IQ](https://microsoft.github.io/copilot-camp/pages/work-iq/)

## 💬 Community

- Ask your questions on our [Discord channel](https://discord.gg/REmjGvvFpW)

### 🚀 Next Up: Continue to [Episode 2](../2-Work-IQ-A2A-for-Context‑Aware-Agentic-Experiences/) for the next step—A2A for Context‑Aware, Agentic Experiences!
